...
// Normalize backend timestamp strings that arrive without timezone information.
// Example: "2026-03-20T11:59:00" should be treated as UTC, then shown in local timezone.
function normalizeServerDateInput(input) {
  ...
  // Datetime without timezone: add Z to make it explicit UTC
...
export function fServerDateTime(date, format) {
  return fDateTime(normalizeServerDateInput(date), format);
}

export function fServerDate(date, format) {
  return fDate(normalizeServerDateInput(date), format);
}

export function fServerTime(date, format) {
  return fTime(normalizeServerDateInput(date), format);
}

export function fLocalTimeZoneShort() {
  try {
...
/** output: Today at 4:20 PM, Yesterday at 9:10 AM, 26 Mar 2026 at 6:45 PM */
export function fDateTimePersonal(date) {
  if (!date) return null;

  const value = dayjs(date);
  if (!value.isValid()) return 'Invalid time value';

  if (value.isSame(dayjs(), 'day')) {
    return `Today at ${value.format('h:mm A')}`;
  }

  if (value.isSame(dayjs().subtract(1, 'day'), 'day')) {
    return `Yesterday at ${value.format('h:mm A')}`;
  }

  return value.format('DD MMM YYYY [at] h:mm A');
}
