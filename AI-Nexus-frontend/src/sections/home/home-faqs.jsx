...
const FAQs = [
  ...
  {
    post: 'Which license is right for you?',
    answer: (
      <Box component="ul" sx={{ pl: 3, listStyleType: 'disc' }}>
        ...
        <li>
          ...
          <Link href="https://www.ai-nexus.io/" target="_blank" rel="noopener" sx={{ mx: 0.5 }}>
            package & license
          </Link>
        </li>
      </Box>
  ...
  {
    post: 'How long is my license valid for?',
    answer: (
      <Box component="ul" sx={{ pl: 3, listStyleType: 'disc' }}>
  ...
  {
    post: 'Which platforms will the template support?',
    answer: (
      <Typography>
  ...
  {
    post: 'For what kind of projects is the Standard license intended?',
    answer: (
      <Typography>
  ...
  {
    post: 'Do you have a free demo to review the code before purchasing?',
    answer: (
      <Typography>
...
export function HomeFAQs({ sx, ...other }) {
  const [expanded, setExpanded] = useState(FAQs[0].post);
    ...
    >
      {FAQs.map((item, index) => (
        <Accordion
          ...
          expanded={expanded === item.post}
          onChange={handleChange(item.post)}
          sx={{
            borderRadius: 2,
            transition: (theme) =>
        ...
        >
          <AccordionSummary
            expandIcon={
              <Iconify
                width={20}
                icon={expanded === item.post ? 'mingcute:minimize-line' : 'mingcute:add-line'}
              />
            }
  ...
  const renderContact = (
    ...
    >
      <m.div variants={varFade().in}>
        <Typography variant="h4">Still have posts?</Typography>
      </m.div>
      ...
      <m.div variants={varFade().in}>
        <Button
          color="inherit"
          variant="contained"
          href="mailto:info@ai-nexus.io?subject=[Feedback] from Customer"
