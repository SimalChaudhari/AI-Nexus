...
export function TagNewEditForm({ currentTag, onCancel }) {
  const theme = useTheme();
  const dispatch = useDispatch();
  const router = useRouter();
  const isEdit = Boolean(currentTag);
          ...
          <Card sx={cardSx}>
            <CardHeader
              ...
              action={
                ...
                >
                  <SvgColor
                    src={`${CONFIG.site.basePath}/assets/icons/navbar/ic-tag.svg`}
            ...
            <Stack spacing={3} sx={{ px: 3, pb: 3 }}>
              <Field.Text
                name="title"
                label="Title"
                placeholder="e.g. Onboarding, Moderation"
                autoFocus={!isEdit}
              />
            </Stack>
          </Card>
