...
export function LearningCoursePlayerView({ course, loading, error }) {
  ...
  const sendProgressUpdate = useCallback((courseId, sectionId, payload, useKeepalive = false, force = false) => {
    ...
    if (!force && !isMarkCompletedOnly) {
      ...
      const knownProgress = Math.max(0, Number(live.watchedSeconds || 0));
            ...
            spotlightrPrepareState === 'pending' ? (
              ...
              onLoadedMetadata={() => {
                ...
                if (
                  ...
                  try {
                    ...
                    const resumeAt = Math.min(
                      resumeSeconds,
                      Number.isFinite(v.duration) ? v.duration : resumeSeconds
                    );
              ...
              onTimeUpdate={() => {
                ...
                if (prog.isPlaying) {
                  ...
                  const maxDelta = isAppleMobileDevice() ? 1.5 : 2.5;
                  if (delta <= maxDelta) {
