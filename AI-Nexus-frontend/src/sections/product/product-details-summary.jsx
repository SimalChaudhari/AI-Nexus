...
export function ProductDetailsSummary({
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (!existProduct) {
        onAddCart?.({ id: data.id, name: data.name, coverUrl: data.coverUrl, price: data.price, quantity: data.quantity || 1 });
      }
      onGotoStep?.(0);
  ...
  const handleAddCart = useCallback(() => {
    try {
      onAddCart?.({ id: values.id, name: values.name, coverUrl: values.coverUrl, price: values.price, quantity: values.quantity || 1 });
    } catch (error) {
      console.error(error);
    }
  }, [onAddCart, values]);
